...
DATA_PATH = "kaspi_coffee_cleaned.csv"
MODEL_POP_PATH = "model_popularity_rf.pkl"
MODEL_REG_PATH = "model_reviews_rf.pkl"
...
def main():
    ...
    df_inventory["sku_label"] = (
        df_inventory["name"].str.slice(0, 60)
        + " | "
        + df_inventory["product_id"]
    )
    ...
    if mode == "Single SKU analysis":
        ...
        # показываем как транспонированную таблицу "поле / значение"
        st.dataframe(
            df_one_inv[detail_cols_unique].T.rename(columns={df_one_inv.index[0]: "value"}),
    ...
    # ====================================
        ...
        brands = sorted(df_inventory["brand_top"].unique())
        ...
        st.dataframe(top_demand, use_container_width=True)
        ...
        st.dataframe(bottom_demand, use_container_width=True)
        ...
        st.dataframe(top_safety, use_container_width=True)
